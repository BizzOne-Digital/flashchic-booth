...
{/* PRICING PREVIEW — Dynamic */}
      <section className="py-24 px-6 bg-[#0d0d0d]">
        <div className="max-w-5xl mx-auto text-center">
          <p className="font-display text-xs tracking-[0.4em] text-[#d4af37] uppercase mb-4">Transparent Pricing</p>
          <h2 className="font-display text-4xl md:text-5xl font-light text-white mb-4">Simple, <span className="gold-text font-semibold">Clear Rates</span></h2>
          <div className="gold-divider mb-12" />
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {pkgs.map((pkg: any) => (
              <div key={pkg._id} className={`p-6 sm:p-8 flex flex-col ${pkg.featured ? 'pricing-featured' : 'luxury-card'}`}>
                {pkg.featured && <div className="text-[10px] tracking-widest text-[#d4af37] uppercase mb-4">Most Popular</div>}
                <p className="font-display text-xs tracking-[0.3em] text-[#d4af37] uppercase mb-4">{pkg.name}</p>
                <div className="flex items-baseline justify-center gap-1 mb-2">
                  <span className="font-display text-4xl sm:text-5xl font-light text-white">${pkg.price}</span>
                  <span className="text-white/40 text-sm">{pkg.unit || '/hr'}</span>
                </div>
                <p className="text-white/40 text-xs tracking-widest mb-6">{pkg.minimum || 2}hr minimum</p>
                <Link
                  href="/pricing"
                  className={`block w-full text-center py-3 text-xs tracking-widest font-sans font-semibold transition-all mt-auto ${
                    pkg.featured
                      ? 'btn-gold'
          ...
          <Link href="/booking" className="btn-gold px-8 sm:px-12 py-4 text-sm tracking-widest font-semibold inline-block w-full sm:w-auto">
            Check Availability
          </Link>
        </div>
      </section>
